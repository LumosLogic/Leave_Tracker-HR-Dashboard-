...
function orgApprovedHtml(req, orgSlug, tempPassword) {
  return WRAP(
    ...
    BODY(`
      ...
      <div style="background:#fff;border:1px solid #bfdbfe;border-radius:8px;padding:16px 20px;margin-bottom:16px;">
        <p style="margin:0 0 10px;font-size:13px;color:#1e40af;font-weight:bold;">Your Login Credentials</p>
        <p style="margin:0 0 6px;font-size:14px;"><span style="color:#64748b;">Login URL:</span> &nbsp;<a href="${process.env.FRONTEND_URL || 'https://hrms.lumoslogic.com'}/login" style="color:#3525cd;">${(process.env.FRONTEND_URL || 'https://hrms.lumoslogic.com').replace('https://', '').replace('http://', '')}/login</a></p>

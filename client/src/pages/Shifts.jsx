...
function ShiftModal({ open, onClose, shift }) {
  ...
  const [form, setForm] = useState(() => isEdit
    ? { name: shift.name, start_time: shift.start_time, end_time: shift.end_time, color: shift.color, description: shift.description || '' }
    : { name: '', start_time: '09:00', end_time: '18:00', color: '#3525cd', description: '' });
  const set = (k, v) => setForm(f => ({ ...f, [k]: v }));

  const mut = useMutation({
    mutationFn: () => isEdit ? apiPut(`/shifts/${shift.id}`, form) : apiPost('/shifts', form),

...
function StructureModal({ open, onClose, userId }) {
  ...
  const empty = { user_id: userId, effective_from: new Date().toISOString().split('T')[0], basic: 0, hra: 0, da: 0, transport_allowance: 0, medical_allowance: 0, other_allowances: 0, pf_employee: 0, pf_employer: 0, esi_employee: 0, esi_employer: 0, professional_tax: 0, tds: 0 };
  ...
  const deductions = ['pf_employee','esi_employee','professional_tax','tds'].reduce((s, k) => s + Number(form[k] || 0), 0);
  ...
  return (
    <Modal open={open} onClose={onClose} title="Set Salary Structure" size="lg"
      ...
      <div className="space-y-5">
        ...
        <div>
          <p className="text-[0.7rem] font-black uppercase tracking-widest text-[#777587] mb-3">Deductions (₹ / month)</p>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            {row('PF (Employee)','pf_employee')}{row('PF (Employer)','pf_employer')}{row('ESI (Employee)','esi_employee')}
            {row('ESI (Employer)','esi_employer')}{row('Professional Tax','professional_tax')}{row('TDS','tds')}
          </div>
          <div className="mt-3 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <TrendingDown size={14} className="text-rose-500" />
              <span className="text-sm font-bold text-rose-600">Deductions: {fmtD(deductions)}</span>
            </div>
            <span className="text-sm font-black text-[#151c27]">Est. Net: {fmtD(gross - deductions)}</span>
          </div>
        </div>
      </div>
    </Modal>
          ...
          <div className="mt-4 pt-4 border-t border-[#f0f3ff]">
            ...
            <div className="rounded-xl bg-gradient-to-r from-[#f0f3ff] to-[#e7eefe] border border-[#c7c4d8] p-4 flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-[#464555]">Working days: {ps.working_days} · Present: {ps.present_days}</p>
              </div>
...
export default function Payroll() {
  ...
  const { data: _psData, isLoading } = useQuery({
    queryKey: ['payslips-all', filterM, filterY],
    queryFn: () => isAdmin ? apiGet('/payroll/payslips/all', { month: String(filterM).padStart(2,'0'), year: filterY }) : apiGet('/payroll/payslips'),
  });
    ...
    <div className={wrap}>
      ...
      {isAdmin && tab === 'structure' ? (
        ...
        <>
          ...
          {isLoading ? <div className="loading"><div className="spinner" />Loading payslips…</div>
            : payslips.length === 0
